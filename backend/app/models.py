...
class CustodianDirectoryEntry(Base):
    """Reusable organization custodian that is not yet attached to a case."""

    __tablename__ = "custodian_directory"
    ...
    employment_status = Column(String(128), nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
